...
My personal collection of agent skills — 85 skills for Pi / Claude Code / Codex / Cursor agents.

Sources consolidated from `~/.agents/skills/` + `~/.pi/agent/skills/` (omarchy-maintenance, rails-conventions).
...
```bash
# for Pi
cp -r tdd ~/.pi/agent/skills/
# for generic agents
cp -r tdd ~/.agents/skills/
```
